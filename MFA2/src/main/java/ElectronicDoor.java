...
public class ElectronicDoor {
 ...
 public static void main(String[] args) {
  SwingUtilities.invokeLater(() -> new ElectronicDoor().createAndShowGUI());
 }
 ...
 private void createAndShowGUI() {
  ...
  logArea.setFont(new Font("Monospaced", Font.PLAIN, 16)); // 设置为粗体
  logArea.setLineWrap(true); // 自动换行

  // JScrollPane 设置
  JScrollPane scrollPane = new JScrollPane(logArea);
  scrollPane.setBorder(BorderFactory.createEmptyBorder());  // 移除默认边框
  scrollPane.setPreferredSize(new Dimension(400, 300)); // 设置日志区域的大小
  scrollPane.setMaximumSize(new Dimension(Integer.MAX_VALUE, 300)); // 设置最大高度，保持扁平化
  ...
  JButton startDeviceButton = new JButton("启动设备");
  JButton exitButton = new JButton("退出");

  // 设置按钮大小和字体
  Dimension buttonSize = new Dimension(550, 100);
  Font buttonFont = new Font("SansSerif", Font.PLAIN, 64); // 设置为粗体和较大字体
  ...
  // 添加按钮到面板
  panel.add(Box.createVerticalStrut(10)); // 增加间距
